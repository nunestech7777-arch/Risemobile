...
CREATE TABLE IF NOT EXISTS public.stock_entries (
    ...
    suggested_price_usd NUMERIC(12, 2) DEFAULT 0.00,
    notes TEXT,
    source VARCHAR(50) DEFAULT 'manual' CHECK (source IN ('manual', 'import', 'external')),
...
CREATE TABLE IF NOT EXISTS public.devices (
    ...
    storage VARCHAR(50) NOT NULL,
    grade_id UUID NOT NULL REFERENCES public.grades(id) ON DELETE RESTRICT,
    -- IMEI/Serial (imei), cor e bateria são OPCIONAIS. O identificador real do
    -- aparelho é o id interno; o IMEI só precisa ser único quando informado
    -- (índice único parcial abaixo). Custo/preço: 0 = não informado.
    color VARCHAR(50),
    battery_health INTEGER CHECK (battery_health >= 0 AND battery_health <= 100),
    imei VARCHAR(30),
    cost_price_usd NUMERIC(12, 2) NOT NULL DEFAULT 0 CHECK (cost_price_usd >= 0),
    suggested_price_usd NUMERIC(12, 2) NOT NULL DEFAULT 0 CHECK (suggested_price_usd >= 0),
    ...
    source VARCHAR(50) DEFAULT 'manual' CHECK (source IN ('manual', 'import', 'external')),
    -- Soft delete (migration 018): aparelho removido fica com status 'Removido' + deleted_at
    deleted_at TIMESTAMPTZ,
    deleted_by VARCHAR(150),
    deletion_reason TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT devices_removed_consistency CHECK ((status = 'Removido') = (deleted_at IS NOT NULL))
);
...
-- Normaliza IMEI/cor vazios para NULL e garante unicidade do IMEI/Serial apenas quando informado
...
CREATE UNIQUE INDEX IF NOT EXISTS unique_devices_imei_when_present
...
GRANT EXECUTE ON FUNCTION public.rpc_sync_external_devices(JSONB, VARCHAR) TO authenticated, anon, service_role;
...
-- ==============================================================================
-- Migration 019: zerar a base de dados operacional (botão "Zerar Base de Dados")
-- ==============================================================================
...
-- ==============================================================================
-- Migration 020: escolher a cor do aparelho na venda
-- ==============================================================================
